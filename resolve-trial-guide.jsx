...
function SADView({ record, onBack, onCheckIn }) {
  ...
  function buildEmailBody() {
    ...
    lines.push(`Role: ${getRoleLabel(answers.role)} | Environment: ${getEnvLabel(answers.envSize)}`);
    lines.push(`Primary goal: ${getPainLabel(answers.painPoint)}`);
    lines.push(`Success definition: ${getSuccessLabel(answers.successDef)}`);
    lines.push(`Overall verdict: ${vd.label}`);
    lines.push(``);
    checkIns.forEach(ci => {
      lines.push(`=== ${ci.week === 5 ? 'End of Trial' : 'Week ' + ci.week} ===`);
      if (ci.scopeChanged && ci.updatedContext) lines.push(`Scope update: ${ci.updatedContext}`);
      (ci.criteria || []).forEach(c => {
        lines.push(`  [${(c.status || 'skipped').toUpperCase()}] ${c.name}${c.comment ? ' — ' + c.comment : ''}`);
      });
      lines.push(``);
    });
    lines.push(`Next steps:`);
    nextSteps.forEach(s => {
      if (s.action) lines.push(`  • ${s.action} | Owner: ${s.owner || 'TBD'} | Date: ${s.date || 'TBD'}`);
    });
