// resolve-trial-guide.jsx
// Requires: react, react-dom only
...
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Chameleon GoToResolve dark token set */
  :root {
    ...
    --brand:       #8552F6;
    --brand-hover: #A37BFB;
    --brand-subtle:rgba(133,82,246,0.14);
    --brand-border:rgba(133,82,246,0.5);

    --yellow:      #F2DD06;
    --yellow-text: #171B1F;

    --text1:       #FFFFFF;
    --text2:       rgba(217,227,233,0.75);
    --text3:       rgba(217,227,233,0.4);

    --border:      rgba(88,121,143,0.45);
    --border2:     rgba(88,121,143,0.7);

    --success:     #2BA95C;
    --success-bg:  rgba(43,169,92,0.12);
    --danger:      #D33A61;
    --danger-bg:   rgba(211,58,97,0.1);
    --warning:     #F2DD06;
    --warning-bg:  rgba(242,221,6,0.1);
  ...
  .progress-dot.active{ background: var(--brand); width: 18px; }

  .question-block { animation: fadeUp 0.32s ease both; }
  @keyframes fadeUp {
  ...
  .option-card.selected .opt-indicator-sq {
    border-color: var(--brand);
    background: var(--brand);
  }
  .opt-text { font-size: 14px; font-weight: 500; color: var(--text1); }

  /* ── Recall panel on intake screen ── */
  .recall-panel {
    margin-top: 32px;
    padding: 20px;
  ...
  .recall-input:focus { border-color: var(--brand); }
  .recall-input::placeholder { color: var(--text3); }
  .recall-msg {
    margin-top: 8px;
    font-size: 12px;
    color: var(--danger);
  ...
  .loading-sub { font-size: 13px; color: var(--text3); font-family: var(--font-mono); }
  .loading-track {
  ...
  @media (max-width: 820px) { .output-main { padding: 24px 0 60px 0; } }
  ...
  .output-title {
    font-size: 26px; font-weight: 700; color: var(--text1); line-height: 1.2;
  }
  ...
  /* ── Sections ── */
  .out-section { margin-bottom: 48px; animation: fadeUp 0.4s ease both; }
  .section-eyebrow {
    font-family: var(--font-mono);
  ...
  .card-danger {
    ...
    border: 1px solid rgba(211,58,97,0.3);
    border-left: 3px solid var(--danger);
    border-radius: var(--r-md);
    padding: 16px 20px;
  }

  .profile-text { font-size: 14px; line-height: 1.75; color: var(--text2); }
  .profile-text strong { color: var(--text1); font-weight: 600; }

  /* ── Week cards ── */
  .weeks-grid { display: flex; flex-direction: column; gap: 16px; }
  .week-card {
    background: var(--surface);
    border: 1px solid var(--border);
    border-radius: var(--r-lg);
  ...
  .week-title { font-size: 14px; font-weight: 600; color: var(--text1); }
  .week-body { padding: 18px 20px; }
  .week-tasks { list-style: none; display: flex; flex-direction: column; gap: 10px; }
  .week-task { display: flex; align-items: flex-start; gap: 10px; font-size: 13.5px; color: var(--text2); line-height: 1.55; }
  ...
  .metrics-table tr:nth-child(even) td { background: rgba(44,53,60,0.4); }
  .metrics-table td:first-child { color: var(--text1); font-weight: 600; }
  .pass-pill {
    display: inline-block;
    background: var(--success-bg); color: var(--success);
    border: 1px solid rgba(43,169,92,0.3);
    padding: 2px 8px; border-radius: var(--r-pill);
    font-size: 11px; font-family: var(--font-mono); white-space: nowrap;
  }

  /* ── Red flags ── */
  .red-flags { display: flex; flex-direction: column; gap: 14px; }
  .flag-title {
    font-size: 13.5px; font-weight: 600; color: var(--text1);
    margin-bottom: 5px; display: flex; align-items: center; gap: 7px;
  }
  .flag-body { font-size: 13px; color: var(--text2); line-height: 1.6; }
  .flag-body strong { color: var(--text1); }

  /* ── Next step ── */
  .next-step-cta { font-size: 16px; font-weight: 600; color: var(--text1); margin-bottom: 10px; }
  .next-step-body { font-size: 14px; color: var(--text2); line-height: 1.7; }
  .next-step-body strong { color: var(--text1); }
  ...
  .save-panel-sub { font-size: 13px; color: var(--text2); margin-bottom: 16px; }
  ...
  /* ── Print ── */
  @media print {
    .sidenav, .topbar-actions, .save-panel, .toast, .btn-ghost, .btn-primary { display: none !important; }
    body { background: #fff !important; color: #111 !important; }
    .output-layout { display: block !important; padding: 0 !important; }
    .output-main { padding: 0 !important; }
    .card, .card-accent, .week-card, .card-danger {
      background: #f7f8fa !important; border-color: #ddd !important;
      break-inside: avoid;
    }
    .week-header { background: #eef0f3 !important; }
    .section-eyebrow, .q-label { color: #5c3abf !important; }
    .section-heading, .output-title, .week-title, .flag-title, .next-step-cta,
    .profile-text strong, .metrics-table td:first-child { color: #111 !important; }
    .profile-text, .week-task, .flag-body, .next-step-body, .metrics-table td { color: #333 !important; }
    .metrics-table th { background: #eef0f3 !important; color: #555 !important; }
    .week-badge { color: #171B1F !important; background: #F2DD06 !important; }
    .print-header { display: block !important; text-align: center; padding: 16px 0 10px; border-bottom: 2px solid #5c3abf; margin-bottom: 20px; }
    .print-footer { display: block !important; position: fixed; bottom: 0; left: 0; right: 0; text-align: center; font-size: 10px; color: #888; padding: 6px; border-top: 1px solid #ccc; }
  }
...
const QUESTIONS = [
  ...
  },
  {
    ...
    text: 'What is the single biggest problem you need to solve in the next 90 days?',
    multi: false,
    options: [
  ...
  },
  {
    ...
    text: 'How will you know this trial was a success? Choose the statement that fits best.',
    multi: false,
    options: [
  ...
  {
    id: 'biggestConcern',
    label: 'Step 6 of 6 — Your Biggest Concern',
    text: 'What would most likely cause you to walk away from this evaluation?',
    multi: false,
    options: [
...
function buildProfileSummary(answers) {
  ...
  return `You are evaluating GoTo Resolve as a ${role} responsible for ${env}. Your current environment includes ${stackStr}. The primary outcome you need to achieve in the next 90 days is ${pain}. You will consider this trial a success when you have demonstrated ${success}. Your most significant evaluation risk is ${concern} — this guide is structured to address that directly.`;
}

/* ── First Value Target ── */
function buildFirstValue(answers) {
  const { role, painPoint, successDef, envSize } = answers;
...
function buildWeeklyPlan(answers) {
  const { role, envSize, painPoint, successDef, biggestConcern, currentStack } = answers;
...
const STORAGE_KEY = 'rg_saved_';

function saveGuide(email, answers) {
  const key = STORAGE_KEY + email.trim().toLowerCase();
...
function RecallPanel() {
  ...
  function handleRecall() {
    const saved = loadGuide(email);
    if (!saved) {
      setMsg('No saved guide found for that email.');
      setMsgType('err');
      return;
    }
    // Dispatch a custom event the App listens for
    window.dispatchEvent(new CustomEvent('rg:recall', { detail: saved.answers }));
  }

  return (
    <div className="recall-panel">
      <div className="recall-panel-title">Returning? Recall your saved guide.</div>
      <div className="recall-row">
        ...
        />
        <button className="btn-primary" onClick={handleRecall} disabled={!email.trim()}>
          Recall <Icon name="RefreshCw" size={14} />
        </button>
      </div>
      {msg && <div className={`recall-msg ${msgType === 'ok' ? 'ok' : ''}`}>{msg}</div>}
    </div>
  );
}

function IntakeQuestion({ question, answers, onAnswer, onContinue }) {
  ...
  const hasSelection = isMulti ? (selected && selected.length > 0) : !!selected;
  ...
  return (
    <div className="question-block">
      ...
      {question.options.map(opt => {
        const isSel = isMulti ? (selected || []).includes(opt.id) : selected === opt.id;
        return (
          <div key={opt.id} className={`option-card ${isSel ? 'selected' : ''}`} onClick={() => handleSelect(opt.id)}>
            <div className={isMulti ? 'opt-indicator-sq' : 'opt-indicator'}>
              {isSel && <Icon name="Check" size={11} color="#fff" strokeWidth={3} />}
            </div>
      ...
      {isMulti && (
        <button className="btn-primary" disabled={!hasSelection} onClick={onContinue} style={{ marginTop: 18 }}>
...
function LoadingView() {
  return (
    <div className="loading-wrap">
      ...
      <div className="loading-sub">Personalizing for your environment</div>
    </div>
  );
}

function SideNav({ onNav, active }) {
  const links = [
    { id: 'profile',    label: 'Evaluation Profile',  icon: 'FileText'    },
    { id: 'firstvalue', label: 'First Value Target',   icon: 'Target'      },
    { id: 'testplan',   label: 'Test Plan',            icon: 'Calendar'    },
    { id: 'metrics',    label: 'Success Metrics',      icon: 'BarChart3'   },
    { id: 'redflags',   label: 'Red Flags',            icon: 'AlertTriangle'},
    { id: 'nextstep',   label: 'Next Step',            icon: 'ArrowRight'  },
  ];
  return (
    <nav className="sidenav">
      <div className="sidenav-logo">GoTo Resolve</div>
      {links.map(l => (
        <a key={l.id} href={`#${l.id}`} className={`sidenav-link ${active === l.id ? 'active' : ''}`} onClick={() => onNav(l.id)}>
  ...
  return (
    <div className="week-card">
      <div className="week-header">
        <span className="week-badge">{week.label}</span>
        ...
        <tbody>
          {metrics.map((m, i) => (
            <tr key={i}>
              <td>{m.name}</td>
              <td>{m.how}</td>
              <td><span className="pass-pill">{m.target}</span></td>
              <td>{m.why}</td>
            </tr>
          ))}
        </tbody>
...
function SavePanel({ answers }) {
  ...
  function handleSave() {
    ...
    saveGuide(trimmed, answers);
    setFeedback('Guide saved! Enter this email on the start screen to recall it anytime.');
    setIsErr(false);
  }

  return (
    <div className="save-panel">
      <div className="save-panel-title">Save your guide</div>
      <div className="save-panel-sub">Enter your email to save this guide. You can recall it anytime from the start screen — even if you close the browser.</div>
      <div className="save-row">
        ...
        />
        <button className="btn-primary" onClick={handleSave} disabled={!email.trim()}>
          <Icon name="Save" size={14} /> Save Guide
        </button>
      </div>
      {feedback && <div className={`save-feedback ${isErr ? 'err' : ''}`}>
        {!isErr && <Icon name="Check" size={13} />} {feedback}
      </div>}
    </div>
  );
}

function OutputView({ answers, onReset }) {
  const [active, setActive] = useState('profile');
  const [toast, setToast] = useState('');

  const profile  = buildProfileSummary(answers);
  const firstVal = buildFirstValue(answers);
  const weeks    = buildWeeklyPlan(answers);
  const metrics  = buildMetrics(answers);
  const flags    = buildRedFlags(answers);
  ...
  return (
    <div className="output-wrap">
      <div className="print-header">
        <strong style={{ fontSize: 16 }}>GoTo Resolve — Trial Evaluation Guide</strong>
      </div>
      <div className="print-footer">
        GoTo Resolve Trial Evaluation Guide | Prepared by GoTo Solutions Consulting | Confidential
      </div>
      ...
      <div className="output-layout">
        ...
        <main className="output-main">
          <div className="output-topbar">
            <div>
              <div className="output-title">Your GoTo Resolve Evaluation Guide</div>
              <div className="output-subtitle">{getRoleLabel(answers.role)} · {getEnvLabel(answers.envSize)}</div>
            </div>
            <div className="topbar-actions">
              <button className="btn-ghost" onClick={onReset}>
                <Icon name="X" size={13} /> Start Over
              </button>
          ...
          <section id="profile" className="out-section">
            <div className="section-eyebrow">Section 01</div>
            <div className="section-heading"><Icon name="FileText" size={17} /> Your Evaluation Profile</div>
            <div className="card"><p className="profile-text">{profile}</p></div>
          </section>
          <div className="divider" />

          <section id="firstvalue" className="out-section">
            <div className="section-eyebrow">Section 02</div>
            <div className="section-heading"><Icon name="Target" size={17} /> Your First Value Target</div>
            <div className="card-accent"><p className="profile-text">{firstVal}</p></div>
          </section>
          <div className="divider" />

          <section id="testplan" className="out-section">
            <div className="section-eyebrow">Section 03</div>
            <div className="section-heading"><Icon name="Calendar" size={17} /> Your Personalized 30-Day Test Plan</div>
            <div className="weeks-grid">{weeks.map((w, i) => <WeekCard key={i} week={w} />)}</div>
          </section>
          <div className="divider" />

          <section id="metrics" className="out-section">
            <div className="section-eyebrow">Section 04</div>
            <div className="section-heading"><Icon name="BarChart3" size={17} /> Success Metrics</div>
            <MetricsTable metrics={metrics} />
          </section>
          <div className="divider" />

          <section id="redflags" className="out-section">
            <div className="section-eyebrow">Section 05</div>
            <div className="section-heading"><Icon name="AlertTriangle" size={17} /> Red Flags to Watch For</div>
            <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 18, lineHeight: 1.65 }}>
              These indicate genuine product fit issues — not learning curve friction. Knowing the difference saves you from both a false positive and a false negative.
            </p>
            <div className="red-flags">
              {flags.map((f, i) => (
                <div key={i} className="card-danger">
                  <div className="flag-title"><Icon name="AlertTriangle" size={14} color="var(--danger)" /> {f.title}</div>
                  <div className="flag-body"><HtmlBody html={f.body} /></div>
                </div>
              ))}
            </div>
          </section>
          ...
          <section id="nextstep" className="out-section">
            <div className="section-eyebrow">Section 06</div>
            <div className="section-heading"><Icon name="ArrowRight" size={17} /> Your Next Step</div>
            <div className="card-accent">
              <div className="next-step-cta">{nextStep.cta}</div>
              <div className="next-step-body"><HtmlBody html={nextStep.body} /></div>
            </div>
          </section>

          <SavePanel answers={answers} />
        </main>
      </div>
...
function ResolveTrialGuide() {
  ...
  useEffect(() => {
    ...
    // Listen for recall events from RecallPanel
    function onRecall(e) {
      setAnswers(e.detail);
      setPhase('loading');
      setTimeout(() => setPhase('output'), 1400);
    }
    window.addEventListener('rg:recall', onRecall);
    return () => {
      window.removeEventListener('rg:recall', onRecall);
  ...
  function handleAnswer(qId, value) {
    ...
    if (!currentQ.multi) {
      ...
      autoAdvanceTimer.current = setTimeout(() => advance(captured), 400);
    }
  }

  function handleContinue() { advance(qIndexRef.current); }
  ...
  if (phase === 'output')  return <OutputView answers={answers} onReset={handleReset} />;

  return (
    <div className="intake-wrap">
      <div className="intake-inner">
        <div className="intake-logo">GoTo Resolve</div>
        <ProgressDots total={totalQ} current={qIndex} />
        <IntakeQuestion
          key={currentQ.id}
          question={currentQ}
          answers={answers}
