...
// Requires: react, react-dom only — no external icon library
...
const ICONS = {
  ...
  FileText:      <><path strokeLinecap="round" strokeLinejoin="round" d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline strokeLinecap="round" strokeLinejoin="round" points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><line x1="10" y1="9" x2="8" y2="9"/></>,
  ...
  AlertTriangle: <><path strokeLinecap="round" strokeLinejoin="round" d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></>,
  ArrowRight:    <><line x1="5" y1="12" x2="19" y2="12"/><polyline strokeLinecap="round" strokeLinejoin="round" points="12 5 19 12 12 19"/></>,
...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    --brand-yellow: #FFD800;
    --brand-navy: #1A1A2E;
    --brand-blue-dark: #1A3C6B;
    --brand-blue-mid: #2563B0;
    --brand-blue-light: #D6E4F7;
    --brand-orange: #E8610A;
    --surface-dark: #111C2B;
    --surface-card: #162032;
    --surface-elevated: #1E2D42;
    --text-primary: #E8EDF4;
    --text-secondary: #8BA4C0;
    --text-muted: #4A6080;
    --border-subtle: rgba(37,99,176,0.25);
    --border-active: rgba(37,99,176,0.6);
    --success: #22C55E;
    --warning: #FFD800;
    --danger: #E8610A;
  }
  ...
  .progress-dot.done { background: var(--brand-yellow); }
  .progress-dot.active { background: var(--brand-yellow); transform: scale(1.35); }

  .question-block { animation: fadeSlideIn 0.38s ease both; }
  ...
  @media (max-width: 860px) { .output-sidenav { display: none; } }
  ...
  .sidenav-link:hover { color: var(--text-primary); background: var(--surface-card); }
  .sidenav-link.active { color: var(--brand-yellow); background: rgba(255,216,0,0.06); }
  ...
  @media (max-width: 860px) { .output-main { padding: 24px 0 60px 0; } }
  ...
  .section-heading svg { color: var(--brand-blue-mid); flex-shrink: 0; }
  ...
  .profile-card strong { color: var(--text-primary); font-weight: 600; }

  /* ── First Value Block ── */
  .first-value-block {
    background: linear-gradient(135deg, rgba(26,60,107,0.4) 0%, rgba(30,45,66,0.6) 100%);
  ...
  .first-value-block strong { color: var(--text-primary); }

  /* ── Week Cards ── */
  .weeks-grid { display: flex; flex-direction: column; gap: 20px; }
  ...
  .week-card:hover { border-color: var(--border-active); }
  ...
  .metrics-table tr:nth-child(even) td { background: rgba(30,45,66,0.3); }
  ...
  /* ── Red Flags ── */
  .red-flags { display: flex; flex-direction: column; gap: 16px; }
  ...
  .red-flag-title svg { color: var(--brand-orange); }

  .red-flag-body { font-size: 13.5px; color: var(--text-secondary); line-height: 1.6; }
  .red-flag-body strong { color: var(--text-primary); }

  /* ── Next Step ── */
  .next-step-card {
    background: linear-gradient(135deg, rgba(37,99,176,0.2) 0%, rgba(26,60,107,0.3) 100%);
    border: 1px solid var(--border-active);
    border-radius: 10px;
    padding: 28px 32px;
  }
  ...
  .next-step-body { font-size: 14.5px; color: var(--text-secondary); line-height: 1.7; }
  .next-step-body strong { color: var(--text-primary); }

  /* ── Print ── */
  @media print {
    .output-sidenav, .export-btn, .intake-wrap, .loading-wrap { display: none !important; }
    body { background: #fff !important; color: #111 !important; }
    .output-layout { display: block !important; max-width: 100% !important; padding: 0 !important; }
    .output-main { padding: 0 !important; }
    .profile-card, .first-value-block, .week-card, .red-flag-card, .next-step-card {
      background: #f8f9fa !important;
      border-color: #ccc !important;
      color: #222 !important;
      break-inside: avoid;
    }
    .week-header { background: #eef2f7 !important; }
    .section-eyebrow, .q-label, .week-badge { color: #1a3c6b !important; background: transparent !important; }
    .section-heading, .output-title, .week-title, .red-flag-title, .next-step-cta { color: #111 !important; }
    .text-primary, .profile-card strong, .option-text { color: #111 !important; }
    .text-secondary, .profile-card, .first-value-block, .week-task,
    .red-flag-body, .next-step-body, .metrics-table td { color: #333 !important; }
    .metrics-table th { color: #555 !important; background: #eef2f7 !important; }
    .weeks-grid .week-card { page-break-inside: avoid; }
    .print-header {
    ...
    .out-section { page-break-inside: avoid; }
  }
...
function IntakeQuestion({ question, answers, onAnswer, onContinue, qIndex }) {
    ...
    <div className="question-block" key={question.id}>
      ...
      {question.options.map(opt => {
        ...
        return (
          ...
          >
            <div className={isMulti ? 'option-check-box' : 'option-check'}>
              {isSelected && (
                <Icon name="Check" size={12} color="#1A1A2E" strokeWidth={3} />
              )}
            </div>
  ...
  return (
    <div className="loading-wrap">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <Icon name="Star" size={22} color="#FFD800" />
        <span className="loading-text">Building your evaluation guide…</span>
      </div>
      <div className="loading-bar-track">
        <div className="loading-bar-fill" />
      </div>
      <div style={{ fontSize: 13, color: 'var(--text-muted)', fontFamily: 'DM Mono, monospace' }}>
        Personalizing for your environment
      </div>
    </div>
  );
}
...
function SideNav({ active, onNav }) {
  const links = [
    { id: 'profile',     label: 'Evaluation Profile',   icon: <Icon name="FileText" size={13} /> },
    { id: 'firstvalue',  label: 'First Value Target',   icon: <Icon name="Target" size={13} /> },
    { id: 'testplan',    label: 'Test Plan',             icon: <Icon name="Calendar" size={13} /> },
    { id: 'metrics',     label: 'Success Metrics',      icon: <Icon name="BarChart3" size={13} /> },
    { id: 'redflags',    label: 'Red Flags',             icon: <Icon name="AlertTriangle" size={13} /> },
    { id: 'nextstep',    label: 'Next Step',             icon: <Icon name="ArrowRight" size={13} /> },
  ];
  return (
    <nav className="output-sidenav">
      <div className="sidenav-logo">GoTo Resolve</div>
      {links.map(l => (
        <a
          key={l.id}
          className={`sidenav-link ${active === l.id ? 'active' : ''}`}
          onClick={() => onNav(l.id)}
          href={`#${l.id}`}
        >
          {l.icon} {l.label}
    ...
    <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid var(--border-subtle)' }}>
      <table className="metrics-table">
        <thead>
          <tr>
            <th>Metric</th>
            <th>How to Measure in Resolve</th>
...
function RedFlagCard({ flag }) {
  return (
    <div className="red-flag-card">
      ...
      <div className="red-flag-body"><HtmlBody html={flag.body} /></div>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="print-header">
        <strong style={{ fontSize: 18 }}>GoTo Resolve</strong>
        <div style={{ fontSize: 12, color: '#555', marginTop: 4 }}>Trial Evaluation Guide</div>
      </div>
      ...
      <div className="output-layout">
        <SideNav active={activeSection} onNav={setActiveSection} />
        <main className="output-main">
          <div className="output-header">
            <div>
              <div className="output-title">Your GoTo Resolve<br />Evaluation Guide</div>
              <div className="output-subtitle">
                Personalized for {getRoleLabel(answers.role)} · {getEnvLabel(answers.envSize)}
              </div>
            </div>
            <button className="export-btn" onClick={handleExport}>
              <Icon name="Download" size={15} /> Export Guide
            </button>
          </div>
          ...
          <section id="profile" className="out-section">
            ...
            <div className="section-heading">
              <Icon name="FileText" size={18} /> Your Evaluation Profile
            </div>
          ...
          {/* Section 2 — First Value */}
          <section id="firstvalue" className="out-section">
            <div className="section-eyebrow">Section 02</div>
            <div className="section-heading">
              <Icon name="Target" size={18} /> Your First Value Target
            </div>
            <div className="first-value-block">{firstVal}</div>
          </section>
          ...
          <section id="testplan" className="out-section">
            ...
            <div className="section-heading">
              <Icon name="Calendar" size={18} /> Your Personalized 30-Day Test Plan
            </div>
          ...
          <section id="metrics" className="out-section">
            ...
            <div className="section-heading">
              <Icon name="BarChart3" size={18} /> Success Metrics
            </div>
          ...
          <section id="redflags" className="out-section">
            ...
            <div className="section-heading">
              <Icon name="AlertTriangle" size={18} /> Red Flags to Watch For
            </div>
            <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 20, lineHeight: 1.65 }}>
              These are honest indicators of genuine product fit issues — not learning curve friction.
              Knowing the difference will save you from both a false negative and a false positive on this evaluation.
            </p>
            <div className="red-flags">
              {redFlags.map((f, i) => <RedFlagCard key={i} flag={f} />)}
            </div>
          </section>
          ...
          <section id="nextstep" className="out-section">
            ...
            <div className="section-heading">
              <Icon name="ArrowRight" size={18} /> Your Next Step
            </div>
...
export default function ResolveTrialGuide() {
  const [phase, setPhase] = useState('intake');
  const [qIndex, setQIndex] = useState(0);
  const [answers, setAnswers] = useState({});
  const autoAdvanceTimer = useRef(null);
  // Store qIndex in a ref so setTimeout callbacks always see the latest value
  ...
  useEffect(() => {
    const existing = document.getElementById('resolve-guide-styles');
    if (!existing) {
      const el = document.createElement('style');
      el.id = 'resolve-guide-styles';
  ...
  function handleAnswer(qId, value) {
    ...
    if (!currentQ.multi) {
      if (autoAdvanceTimer.current) clearTimeout(autoAdvanceTimer.current);
      // Capture current index in a local variable — safe from stale closure
      const capturedIndex = qIndexRef.current;
      autoAdvanceTimer.current = setTimeout(() => advance(capturedIndex), 400);
    }
  }
